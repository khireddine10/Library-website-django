...
# view that will return list of all memoires
def MemoireListView(request):
    memoire_list = Memoire.objects.all()
    return render(request, 'catalog/memoire_list.html', locals())

# this list of all student that borrowed a book
@login_required
...
#This view return detail of a particular memoire
#it also accepts a parameter pk that is the id  i.e. primary_key of memoire to identify it
#get_object_404 if object is not found then return 404 server error
#locals return a dictionary of loacl varibles
def MemoireDetailView(request, pk):
    memoire = get_object_or_404(Memoire, id=pk)
    reviews=Reviews.objects.filter(book=memoire).exclude(review="none")
    try:
        stu = Student.objects.get(roll_no=request.user)
        rr=Reviews.objects.get(review="none")
    except:
        pass
    return render(request, 'catalog/memoire_detail.html', locals())

# book create view it require login
@login_required
...
# memoire create view it require login
@login_required
def MemoireCreate(request):
    if not request.user.is_superuser:
        return redirect('index')
    form = MemoireForm()
    if request.method == 'POST':
        form = MemoireForm(data=request.POST)
        if form.is_valid():
            form.save()
            return redirect(index)
    return render(request, 'catalog/form.html', locals())

# this for update specific book
@login_required
...
# this for update specific memoire

@login_required
def memoireUpdate(request, pk):
    if not request.user.is_superuser:
        return redirect('index')
    obj = Memoire.objects.get(id=pk)
    form = MemoireForm(instance=obj)
    if request.method == 'POST':
        form = MemoireForm(data=request.POST, files=request.FILES, instance=obj)
        if form.is_valid():
            obj = form.save(commit=False)
            obj.save()
            return redirect(index)
    return render(request, 'catalog/form.html', locals())
# this for delete specific memoire
@login_required
def MemoireDelete(request, pk):
    if not request.user.is_superuser:
        return redirect('index')
    obj = get_object_or_404(Memoire, pk=pk)
    obj.delete()
    return redirect('index')	


# this function it return all information about studnet
@login_required
...
# this view for create a student
@login_required
...
# this view about update student information
@login_required
...
# this view about delete a student
@login_required
def StudentDelete(request, pk):
    obj = get_object_or_404(Student, pk=pk)
    obj.delete()
    return redirect('index')
# this view is for student list
@login_required
